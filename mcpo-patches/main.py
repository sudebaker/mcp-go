...
def process_tool_response(result: CallToolResult) -> list:
    ...
    for content in result.content:
        if isinstance(content, types.TextContent):
            ...
            # Handle embedded resources (PDFs, files, etc.)
            resource = content.resource
            if isinstance(resource, types.TextResourceContents):
                # Handle text-based resources (includes base64-encoded PDFs)
                mime_type = getattr(resource, "mimeType", "application/octet-stream")
                uri = getattr(resource, "uri", "")
                text_data = getattr(resource, "text", "")

                # For PDF and other binary formats, return as data URL
                if mime_type in [
                    "application/pdf",
                    "application/zip",
                    "application/octet-stream",
                ]:
                    # Return as data URL for browser/client to handle
                    resource_data = f"data:{mime_type};base64,{text_data}"
                    response.append(
                        {
                            "type": "resource",
                            "uri": uri,
                            "mimeType": mime_type,
                            "data": resource_data,
                            "size": len(text_data) if text_data else 0,
                        }
                    )
                else:
                    # For text resources, return the text directly
                    response.append(
                        {
                            "type": "resource",
                            "uri": uri,
                ...
                # Handle binary blob resources
                mime_type = getattr(resource, "mimeType", "application/octet-stream")
                uri = getattr(resource, "uri", "")
                blob_data = getattr(resource, "blob", "")

                # Return as data URL
                resource_data = f"data:{mime_type};base64,{blob_data}"
                response.append(
                    {
                        "type": "resource",
                        "uri": uri,
                ...
                logger.warning(f"Unknown resource type: {type(resource)}")
                response.append(
                    {
                        "type": "resource",
